...
@Component({
  ...
  template: `
    <div class="shell">
      <aside class="sidebar">
        <div class="sidebar-brand">
          ...
          <div class="brand-info">
            ...
            <span class="brand-sub">Auditoría</span>
          </div>
        </div>
        <div class="badge-wrap"><span class="role-badge"><i class="pi pi-eye"></i> Auditor · solo lectura</span></div>
        <div class="sidebar-profile">
          <div class="profile-avatar">{{ initial() }}</div>
          <div class="profile-info"><span class="profile-name">{{ fullName() }}</span></div>
        </div>
        <div class="sidebar-section-label">CONSULTA</div>
        ...
        <div class="sidebar-bottom">
          <a [routerLink]="'/auditor/soporte'" routerLinkActive="active" class="nav-item">
      ...
      <div class="main-wrapper">
        <header class="topbar">
          <span class="bc-current">Auditoría · consulta de indicadores y rúbricas</span>
  ...
  styles: [`
    ...
    .role-badge { display: inline-flex; align-items: center; gap: 5px; font-size: 10px; font-weight: 700; color: var(--badge-open, #16a34a); background: rgba(22,163,74,0.1); border: 1px solid rgba(22,163,74,0.25); padding: 4px 10px; border-radius: 20px; }
    ...
    .nav-item.active { background: rgba(22,163,74,0.15); color: #fff; font-weight: 600; }
...
export class AuditorLayoutComponent {
  navItems: NavItem[] = [
    { label: 'Dashboard', icon: 'pi-home', route: '/auditor/dashboard' },
    { label: 'Indicadores y rúbricas', icon: 'pi-chart-bar', route: '/auditor/indicadores' },
  ];
  constructor(private auth: AuthService) {}
  fullName() { return this.auth.user()?.name?.trim() || 'Auditor'; }
